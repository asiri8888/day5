    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        ...
        # Writing nothing to it results in an empty file.
                    ...
                    with open(save_path.rstrip(), 'wb') as f: # Open in binary write mode
                        while True:
                            bytes_read = conn.recv(4096) # Receive data in chunks of 4096 bytes
                            print(bytes_read)
                            if not bytes_read:  # if recv() returns empty bytes, 
                                break
                            f.write(bytes_read) # Write the received bytes to the file  
